...
function playerAction(draw){
  ...
  if(isHost){
    ...
    if(draw){
      if(v>=21){setStatus("Cái đã đạt 21 hoặc Quắc.");return}
...
function render(){
  ...
  // Cái chỉ được xem bài người chơi sau khi chính Cái nhấn Dừng.
  // Trong lúc Cái còn đang chơi, tuyệt đối không hiển thị bài của người chơi,
  // kể cả những người đã nhấn Dừng trước đó.
  if(isHost && state.phase==="playing" && state.dealerDone){
